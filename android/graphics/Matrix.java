...
public class Matrix {
	...
	* scale是缩放，skew是错切（canvas变换中有讲过），trans是平移，persp代表透视
	*/
	...
	/**
	 * 构造函数有两个，第一个是直接创建一个单位矩阵，
	 * 第二个是根据提供的矩阵创建一个新的矩阵（采用deep copy）
	 */
     ...
     * perspective.
     *
     * @return Whether the matrix is affine.  判断是否是仿射矩阵  affine：仿射；仿射的；变换和仿射
     */

	/*
		首先来看看什么是仿射变换。仿射变换其实就是二维坐标到二维坐标的线性变换，
		保持二维图形的“平直性”（即变换后直线还是直线不会打弯，圆弧还是圆弧）和“平行性”
		（指保持二维图形间的相对位置关系不变，平行线还是平行线，而直线上点的位置顺序不变），
		可以通过一系列的原子变换的复合来实现，原子变换就包括：平移、缩放、翻转、旋转和错切。
		这里除了透视可以改变z轴以外，其他的变换基本都是上述的原子变换，
		所以，只要最后一行是0,0,1则是仿射矩阵。
	*/
	...
	*/
    ...
    /** Set the matrix to translate by (dx, dy).  设置平移效果，参数分别是x，y上的平移量。 */ 
    public void setTranslate(float dx, float dy) {
